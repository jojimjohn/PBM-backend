...
const purchaseOrderSchema = Joi.object({
  orderNumber: Joi.string().optional(), // Ignored — generated by backend
...
router.post('/',
  ...
  async (req, res) => {
    try {
      ...
      // orderNumber is always backend-generated — never trust client-supplied value
      ...
      const result = await db.transaction(async (trx) => {
        // Generate sequence number atomically inside transaction
        const now = new Date();
        const orderNumber = await getNextSequence(trx, 'PO', now.getFullYear(), now.getMonth() + 1);

        const orderData = {
          orderNumber,
          reference_number: orderFields.reference_number || null,
          supplierId: orderFields.supplierId,
          supplierName: supplier.name,
          supplierPhone: supplier.phone || null,
          supplierEmail: supplier.email || null,
          supplierAddress: supplier.address || null,
          orderDate: orderFields.orderDate || now,
